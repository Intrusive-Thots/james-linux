...
class TestSedgeCoreIdeaEngine(unittest.TestCase):
    """
    Tests for the SEDGE components to verify self-evolving behavior
    and mathematically prove the core idea functionality.
    """
    ...
    def test_edge_model_and_scoring(self):
        """Test Edge model, defaults, and utility score calculation."""
    ...
    def test_decision_graph_core(self):
        """Test DecisionGraph node and edge management, and best next selection."""
    ...
    def test_learning_engine_feedback(self):
        """Test LearningEngine updates edge weights based on outcomes."""
        edge = Edge(from_node="A", to_node="B")
        self.graph.add_edge(edge)
    ...
    def test_decision_engine_stochastic_selection(self):
        """Test DecisionEngine uses weighted stochastic selection."""
    ...
    def test_self_evolving_loop_and_convergence(self):
        """Test SelfEvolvingAgent execution loop and graph convergence over time."""
    ...
    def test_edge_score_zero_division_prevention(self):
        """Test Edge score calculation handles zero failure weight."""
        edge = Edge(
    ...
    def test_decision_engine_zero_utility_fallback(self):
        """Test DecisionEngine handles edge cases where total weight is zero."""
        decision_engine = DecisionEngine(self.graph)
    ...
    def test_graph_path_score(self):
        """Test DecisionGraph calculates average utility score of a path correctly."""
        self.graph.add_node(Node("A", "state"))
        self.graph.add_node(Node("B", "state"))
        self.graph.add_node(Node("C", "state"))

        edge1 = Edge("A", "B", success_weight=2.0, failure_weight=1.0)
        edge2 = Edge("B", "C", success_weight=4.0, failure_weight=1.0)

        self.graph.add_edge(edge1)
        self.graph.add_edge(edge2)

        # Path "A" -> "B" -> "C"
        # edge1 score = 2.0 / (1.0 + 1e-6) ~= 2.0
        # edge2 score = 4.0 / (1.0 + 1e-6) ~= 4.0
        # Avg = (2.0 + 4.0) / 2 = 3.0

        avg_score = self.graph.get_path_score(["A", "B", "C"])
        self.assertAlmostEqual(avg_score, 3.0, places=3)

        # Broken path
        broken_score = self.graph.get_path_score(["A", "C"])
        self.assertEqual(broken_score, 0.0)

        # Single node
        single_score = self.graph.get_path_score(["A"])
