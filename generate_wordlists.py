...
def gen_wifi_common():
    ...
    bases = [
        "password", "internet", "wifi", "wireless", "network", "home",
        "house", "family", "welcome", "connect", "guest", "admin",
        "router", "netgear", "linksys", "comcast", "spectrum", "verizon",
        "xfinity", "att", "tmobile", "sprint", "frontier", "cox",
        "charter", "centurylink", "optimum", "fios",
        "love", "dragon", "master", "monkey", "shadow", "sunshine",
        "princess", "football", "baseball", "soccer", "hockey",
        "batman", "superman", "letmein", "trustno1", "access",
        "hello", "charlie", "donald", "jordan", "thomas",
    ]
    ...
    for base in bases:
        ...
        # Common suffixes
        for suffix in ["1", "12", "123", "1234", "12345", "!", "!!", "01", "99", "2024", "2025", "2026"]:
    ...
    words.extend([
        ...
        "01234567", "13131313", "14141414", "15151515", "10101010",
        "98765432", "11112222", "12345679", "123456789", "1234567890",
        "0123456789", "9876543210", "1111111111", "0000000000",
    ])

    # Keyboard patterns (8+ chars)
    words.extend([
        "qwerty12", "qwerty123", "qwertyui", "qwertyuiop",
        "asdfghjk", "asdfghjkl", "zxcvbnm1", "qazwsxedc",
        "1qaz2wsx", "1q2w3e4r", "q1w2e3r4", "zaq12wsx",
        "poiuytre", "lkjhgfds", "mnbvcxza",
        "asdfjkl;", "qweasdzxc", "1234qwer", "qwer1234",
        "pass1234", "abcd1234", "abcdefgh", "abcdefg1",
    ])

    # Phone number patterns (common 8-digit WiFi passwords)
    for area in ["212", "310", "312", "404", "415", "512", "617", "702", "713", "718", "786", "818", "917"]:
...
def gen_wifi_names():
    ...
    for name in name_bases:
        ...
        for suffix in ["123", "1234", "12345", "!", "2024", "2025", "2026", "wifi1", "net1"]:
    ...
    ssid_patterns = [
        "mywifi", "mynetwork", "myhome", "myhouse", "myinternet",
        "homewifi", "homenet", "ourwifi", "ournetwork", "ourhome",
        "guestwifi", "guestnet", "guestpass", "guestaccess",
        "wifipassword", "wifipass", "wifi1234", "wifikey",
    ]
...
def gen_default_creds():
    ...
    router_defaults = [
        # Netgear
        "password", "admin", "1234", "comcast",
        # Linksys
        "admin", "linksys", "default",
        # TP-Link
        ...
        "ubnt", "admin",
        # Cisco
        "cisco", "admin", "Cisco", "Cisco123",
        # Mikrotik
        "admin", "",
        # Arris
        "admin", "password", "motorola",
        # Belkin
        "admin", "belkin",
        # ZTE
        "admin", "zte", "user",
        # Huawei
        "admin", "huawei", "Admin@123",
        # Generic IoT
        "root", "toor", "default", "pass", "test",
        "admin123", "root123", "password1", "12345",
        "changeme", "letmein", "master", "access",
    ]

    # Service defaults
    service_defaults = {
        "ssh": ["root:root", "root:toor", "admin:admin", "pi:raspberry",
                "ubuntu:ubuntu", "user:user", "test:test", "guest:guest",
                "root:password", "root:123456", "admin:password", "admin:123456"],
        "ftp": ["anonymous:", "ftp:ftp", "admin:admin", "user:user",
                "test:test", "root:root", "ftpuser:ftpuser"],
        "mysql": ["root:", "root:root", "root:mysql", "root:password",
                  "root:123456", "admin:admin", "mysql:mysql", "dbadmin:dbadmin"],
        "postgres": ["postgres:postgres", "postgres:password", "postgres:123456",
                     "admin:admin", "pgadmin:pgadmin"],
        "telnet": ["admin:admin", "root:root", "user:user",
                   "admin:password", "admin:1234", "root:"],
        "snmp": ["public", "private", "community", "snmp", "default",
                 "monitor", "manager", "admin"],
    }
    ...
    for svc, pairs in service_defaults.items():
        ...
        passes = list(set(p.split(":")[1] for p in pairs if ":" in p and p.split(":")[1]))
...
def gen_hybrid_wifi():
    ...
    for f in ["wifi-wpa-top62.txt", "wifi-wpa-top447.txt", "wifi-wpa-top4800.txt",
              "wifi-custom-patterns.txt", "wifi-name-patterns.txt"]:
...
def gen_web_fuzzing():
    ...
    paths = [
        ...
        "admin.php", "admin/login", "admin/index.php", "cpanel", "phpmyadmin",
        "dashboard", "panel", "manage", "management", "console", "portal",
        "webmail", "mail", "email", "owa", "exchange",
        # API paths
        ...
        "settings.py", "settings.json", "database.yml",
        ...
        "backup.bak", "site.zip", "www.zip", "htdocs.zip",
        # CMS specific
        ...
        "kubernetes", "k8s", ".kube/config",
        # Common directories
        "images", "img", "css", "js", "static", "assets",
        "uploads", "files", "documents", "docs", "media",
...
def gen_subdomain_custom():
    ...
    subs = [
        ...
        "cloud", "api", "app", "staging", "admin", "forum",
        "news", "lab", "labs", "git", "gitlab", "cdn", "assets",
        "static", "media", "images", "img", "db", "database",
        "docs", "doc", "status", "monitor", "grafana", "prometheus",
        "jenkins", "ci", "cd", "deploy", "build", "registry",
        "docker", "k8s", "kubernetes", "auth", "sso", "login",
        "oauth", "accounts", "billing", "payments", "pay",
        "internal", "intranet", "extranet", "private", "corp",
        "office", "exchange", "owa", "autodiscover", "lyncdiscover",
        "sip", "meet", "conference", "chat", "slack", "teams",
        "jira", "confluence", "wiki", "help", "helpdesk",
        "tickets", "crm", "erp", "hr", "finance",
        "backup", "bak", "old", "legacy", "archive",
        "sandbox", "stage", "stg", "uat", "qa", "preprod", "pre",
        "demo", "beta", "alpha", "canary", "edge",
        "proxy", "gateway", "lb", "loadbalancer", "cache",
        "search", "elastic", "elasticsearch", "kibana", "logstash",
        "redis", "memcached", "rabbitmq", "kafka", "mq",
        "mysql", "postgres", "postgresql", "mongo", "mongodb",
        "s3", "storage", "blob", "bucket", "files",
        "analytics", "tracking", "stats", "metrics",
        "mobile", "ios", "android", "wap",
