...
class TroubleshootTab(QWidget):
    ...
    def view_logs(self):
        ...
        self.worker = WorkerThread(
            self.orchestrator.layer.run, "dmesg | tail -n 50"
        )
    ...
    def kill_processes(self):
        ...
        self.worker = WorkerThread(
            ...
            "killall airodump-ng aireplay-ng aircrack-ng hashcat hostapd dnsmasq 2>/dev/null",
            sudo=True,
        )
        self.worker.finished.connect(
            lambda r: self._on_check_done("Processes killed.")
        )
        self.worker.start()
