...
class AutoKarmaWorker(QThread):
    ...
    def _do_workflow(self):
        ...
        if not ifaces:
            ...
            return
        mon_iface = next(
            (i["interface"] for i in ifaces
             if self.orchestrator.net_guard.check_monitor_safe(i["interface"])[0]),
            None
        )
        if not mon_iface:
...
class WiFiArsenalTab(QWidget):
    ...
    def _build_recon_tab(self) -> QWidget:
        ...
        self.ap_table.setHorizontalHeaderLabels(["BSSID", "ESSID", "CH", "ENC", "PWR", "SIG"])
        ...
        self.client_table.setHorizontalHeaderLabels(["MAC", "Associated AP", "Probed SSIDs", "PWR"])
    ...
    def _build_karma_tab(self) -> QWidget:
        ...
        self.karma_creds_tbl.setHorizontalHeaderLabels(["IP", "Captured Payload"])
    ...
    def _disable_monitor(self):
        ...
        self.worker.finished.connect(lambda _: (
            self.main_window._set_idle(True), self._refresh_interfaces()
        ))
        self.worker.start()
    ...
    def _do_poll(self):
        try:
            r = self.orchestrator.layer.run("cat /tmp/james_recon-01.csv 2>/dev/null", timeout=3)
            if r.returncode == 0:
        ...
        for line in csv_text.splitlines():
            ...
            if in_clients:
                ...
                if len(parts) >= 14 and len(parts[0]) == 17:
                    aps.append({
                        "bssid": parts[0], "power": parts[8],
                        "channel": parts[3].strip(), "privacy": parts[5].strip(),
                        "essid": parts[13].strip(), "signal": parts[8],
                    })
    ...
    def _populate_ap_table(self, aps: list):
        ...
        for ap in aps:
            ...
            values = [ap["bssid"], ap["essid"], ap["channel"],
    ...
    def _populate_client_table(self, clients: list):
        ...
        for c in clients:
            ...
            for col, val in enumerate([c["mac"], c["bssid"], c["probes"], c["power"]]):
                item = QTableWidgetItem(val)
                item.setFlags(Qt.ItemIsSelectable | Qt.ItemIsEnabled)
    ...
    def _on_ap_selected(self):
        ...
        if row < 0:
            return
        def cell(c): return (self.ap_table.item(row, c) or QTableWidgetItem("")).text()
    ...
    def _ap_context_menu(self, pos: QPoint):
        ...
        menu.addAction("Copy BSSID", lambda: (
            __import__("PyQt5.QtWidgets", fromlist=["QApplication"]).QApplication.clipboard().setText(bssid)
        ))
    ...
    def _client_context_menu(self, pos: QPoint):
        ...
        mac = (self.client_table.item(row, 0) or QTableWidgetItem("")).text()
        menu = QMenu(self)
        menu.addAction("Copy MAC", lambda: (
            __import__("PyQt5.QtWidgets", fromlist=["QApplication"]).QApplication.clipboard().setText(mac)
        ))
    ...
    def _capture_handshake(self):
        ...
        self.main_window._set_idle(False)
        self.main_window._append_log(
            f"Capturing handshake from {self.selected_essid} ({self.selected_bssid})", "INFO"
        )
        ...
        def _do():
            ...
            for _ in range(3):
                ...
                if self.orchestrator.aircrack.check_handshake(cap + "-01.cap", bssid):
    ...
    def _browse_capture(self):
        path, _ = QFileDialog.getOpenFileName(
            self, "Select Capture File", str(Path.home()),
    ...
    def _update_karma_stats(self, status: dict):
        self._get_kpi_val(self._k_clients).setText(str(status.get("client_count", 0)))
        self._get_kpi_val(self._k_dns).setText(str(status.get("dns_count", 0)))
        self._get_kpi_val(self._k_creds).setText(str(status.get("cred_count", 0)))
    ...
    def _on_karma_done(self, success: bool):
        ...
        self.btn_karma_stop.setEnabled(False)
        self._karma_log("Campaign complete" if success else "Campaign failed/aborted")
