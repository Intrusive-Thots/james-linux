...
class SetupTab(QWidget):
    ...
    def _build_ui(self):
        ...
        desc = QLabel(
            "Bridge a local ethernet connection with the active internet connection to provide internet access to downstream devices (like a hardware Pineapple)."
        )
    ...
    def setup_ics(self):
        ...
        def _do_bridge():
            self.orchestrator.layer.run(
                f"nmcli connection add type ethernet ifname {lan} ipv4.method shared con-name Shared-Ethernet",
                sudo=True,
            )
            self.orchestrator.layer.run(
                f"nmcli connection up Shared-Ethernet", sudo=True
            )
            return {"success": True}
