...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self._targets_file = (
            Path.home() / ".james" / "loot" / "known_targets.json"
        )
        self._load_targets()
    ...
    def _build_ui(self):
        ...
        self.tabs.setTabToolTip(
            0, "Conversational AI — talk to JAMES in plain English"
        )
        ...
        self.tabs.setTabToolTip(
            4, "Wi-Fi auditing — monitor mode, deauth, AutoPwn"
        )
        ...
        shortcut_hint = QLabel(
            "Ctrl+1-7: Tabs │ Ctrl+K: Kill │ Ctrl+R: Reboot │ Ctrl+/: Chat"
        )
    ...
    def _notify_tab(self, tab_index: int):
        ...
        base = self._tab_base_labels.get(
            tab_index, self.tabs.tabText(tab_index)
        )
    ...
    def _update_worker_count(self):
        ...
        self._status_workers.setText(
            f"⚙ {active} worker{'s' if active != 1 else ''}"
        )
        if active > 0:
            self._status_workers.setStyleSheet(
    ...
    def _qa_send(self, cmd: str):
        ...
        if "{target}" in cmd:
            ...
            if target:
                ...
                show_toast(
                    self,
                    "Set a target first: type 'set target <IP>'",
                    "warning",
                    2500,
                )
                self.tabs.setCurrentIndex(0)
    ...
    def _make_context_strip(self) -> QWidget:
        ...
        prefix.setStyleSheet(
            "color: #1a3050; font-size: 10px; font-weight: bold; letter-spacing: 1px;"
        )
        ...
        try:
            ...
            if hasattr(self, "net_guard_label"):
                if guard["is_wifi"] and guard["bssid"]:
                    ...
                    self.net_guard_label.setText("⚠️")
                    self.net_guard_label.setToolTip(
                        "No connection detected — guard inactive"
                    )
                    self.net_guard_label.setStyleSheet(
    ...
    def _make_header(self) -> QWidget:
        ...
        title.setStyleSheet(
            "font-size: 24px; font-weight: bold; color: #00f0ff; letter-spacing: 4px; background: transparent;"
        )
        lay.addWidget(title)

        subtitle = QLabel("Autonomous Pentesting Agent")
        subtitle.setStyleSheet(
            "color: #3a5a7a; font-size: 11px; letter-spacing: 1px; background: transparent;"
        )
        ...
        self.clock_label.setStyleSheet(
            "color: #3a5a7a; font-size: 12px; background: transparent;"
        )
        ...
        self.reboot_btn.setToolTip(
            "Reboot JAMES: kill tools, clear context, re-init everything"
        )
        ...
        self.kill_btn.setToolTip(
            "Kill all tools, restore interfaces, reconnect Wi-Fi"
        )
        ...
        self.remote_btn.setToolTip(
            "Start/stop web remote control (access JAMES from another device)"
        )
        ...
        self.gui_remote_btn.setToolTip(
            "Stream the full JAMES GUI to a browser via VNC (any device, no install)"
        )
    ...
    def _make_dashboard_tab(self) -> QWidget:
        ...
        self.palette_target.lineEdit().setPlaceholderText(
            "Target (IP / domain)"
        )
        ...
        self.cmd_input.setPlaceholderText(
            "Shell command (optional — use buttons instead!)"
        )
            ...
            for prefix in TARGET_CMDS:
                if cmd == prefix:
                    if not target:
                        self.palette_target.setFocus()
                        show_toast(
                            self, "Enter a target first →", "warning", 2000
                        )
                        return
    ...
    def _make_oneclick_tab(self) -> QWidget:
        ...
        self._skill_groups: list[
            tuple[QGroupBox, list[QWidget], list[str]]
        ] = []
    ...
    def _build_skill_cards(self):
        all_skills = {
            s: self.orch.load_skill(s) for s in self.orch.list_skills()
        }
        ...
        for cat_name, skill_names in SKILL_CATEGORIES.items():
            ...
            if card_widgets:
                ...
                self._skill_groups.append(
                    (cat_group, card_widgets, card_keywords)
                )
    ...
    def _make_skill_card(self, skill_name: str, skill_data: dict) -> tuple:
        ...
        display_name = (
            skill_data.get("name", skill_name).replace("_", " ").title()
        )
        ...
        if desc:
            ...
            desc_lbl.setStyleSheet(
                "color: #6a8aaa; font-size: 11px; font-style: italic;"
            )
        ...
        run_btn.clicked.connect(
            lambda _, s=skill_data, f=input_fields: self._run_oneclick_test(
                s, f
            )
        )
        ...
        for var_name, combo_box in fields.items():
            ...
            if not val:
                QMessageBox.warning(
                    self,
                    "Missing Input",
                    f"Please provide a value for '{var_name}'.",
                )
                return
            context[var_name] = val

        self._term_print(
            f"[TEST] Starting 1-click test: {skill_data.get('name')} ..."
        )
        w = WorkerThread(self.orch.execute_skill_steps, skill_data, context)
        w.finished.connect(
            lambda _: self._term_print(
                f"[TEST] Finished 1-click test: {skill_data.get('name')}"
            )
        )
        w.error.connect(
            lambda e: self._term_print(f"[ERROR] Test failed: {e}")
        )
    ...
    def _make_recon_tab(self) -> QWidget:
        ...
        self.recon_target.lineEdit().setPlaceholderText(
        ...
        scan_actions_1 = [
            (
                "🔍 Quick Scan",
                self._do_quick_scan,
                "Fast nmap scan (top 1000 ports)",
            ),
            (
                "📋 Full Scan",
                self._do_full_scan,
                "Deep scan with service detection + scripts",
            ),
            (
                "🖥️ OS Detect",
                lambda: self._do_recon_cmd("os detect"),
                "OS fingerprinting via nmap",
            ),
            (
                "⚡ Masscan",
                lambda: self._do_recon_cmd("masscan"),
                "All 65535 ports at max speed",
            ),
        ]
        ...
        scan_actions_2 = [
            (
                "👁️ Stealth",
                lambda: self._do_recon_cmd("stealth recon"),
                "Passive OSINT chain",
            ),
            (
                "🌐 OSINT",
                lambda: self._do_recon_cmd("osint"),
                "Email + subdomain harvest",
            ),
            (
                "📡 DNS Enum",
                lambda: self._do_recon_cmd("dns enum"),
                "DNS records + zone transfer",
            ),
            (
                "🔓 Vuln Scan",
                lambda: self._do_recon_cmd("run skill vuln_scan"),
        ...
        self.recon_table.horizontalHeader().setSectionResizeMode(
            QHeaderView.Stretch
        )
        ...
        self.recon_table.customContextMenuRequested.connect(
            self._recon_context_menu
        )
    ...
    def _recon_context_menu(self, pos):
        ...
        if host_text:
            ...
            copy_ip.triggered.connect(
                lambda: QApplication.clipboard().setText(host_text)
            )

            menu.addSeparator()

            full_scan = menu.addAction(f"📋 Full Scan → {host_text}")
            full_scan.triggered.connect(
                lambda: self._run_agent_cmd(f"full scan {host_text}")
            )

            web_pwn = menu.addAction(f"🌐 Web Pwn → {host_text}")
            web_pwn.triggered.connect(
                lambda: self._run_agent_cmd(f"web pwn http://{host_text}")
            )

            brute = menu.addAction(f"🔓 Brute → {host_text}")
            brute.triggered.connect(
                lambda: self._run_agent_cmd(f"brute {host_text}")
            )

            menu.addSeparator()

            smb_act = menu.addAction(f"📂 SMB Enum → {host_text}")
            smb_act.triggered.connect(
                lambda: self._run_agent_cmd(f"smb enum {host_text}")
            )

            nikto_act = menu.addAction(f"🌐 Nikto Scan → {host_text}")
            nikto_act.triggered.connect(
                lambda: self._run_agent_cmd(f"nikto http://{host_text}")
            )

            dir_act = menu.addAction(f"📁 Dir Bust → {host_text}")
            dir_act.triggered.connect(
                lambda: self._run_agent_cmd(f"gobuster http://{host_text}")
            )

            menu.addSeparator()

            set_target = menu.addAction(f"🎯 Set as Target")
            set_target.triggered.connect(
                lambda: self._set_target_from_menu(host_text)
            )
    ...
    def _do_recon_cmd(self, cmd: str):
        ...
        if not target:
            QMessageBox.warning(
                self, "No Target", "Enter a target IP/domain first."
            )
            return
        if target not in self.known_targets:
            self.known_targets.add(target)
    ...
    def _make_wifi_tab(self) -> QWidget:
        ...
        self.autopwn_btn.setStyleSheet(
            "background-color: #5a1a1a; color: #ff5555; font-weight: bold;"
        )
        ...
        wifi_wl_browse.clicked.connect(
            lambda: self._browse_combo(self.wifi_wl_combo, "*")
        )
    ...
    def _make_cracking_tab(self) -> QWidget:
        ...
        self.cap_path.lineEdit().setPlaceholderText(
            "Select or browse for .cap file"
        )
        ...
        cap_browse.clicked.connect(
            lambda: self._browse_combo(self.cap_path, "*.cap *.hccapx *.pcap")
        )
        ...
        wl_browse.clicked.connect(
            lambda: self._browse_combo(self.wl_path, "*")
        )
        ...
        self.hash_path.setPlaceholderText(
            "Path to hash file or paste hash directly"
        )
        ...
        hwl_browse.clicked.connect(
            lambda: self._browse_combo(self.hash_wl_path, "*")
        )
        ...
        out_label.setStyleSheet(
            "color: #5a8aaa; font-weight: bold; font-size: 12px;"
        )
        ...
        if captures_dir.exists():
            ...
            for cap in caps:
                if cap.suffix.lower() in (
                    ...
                    self.cap_path.addItem(
                        f"{cap.name}  ({cap.stat().st_size // 1024}KB)",
                        str(cap),
                    )
        if self.cap_path.count() == 0:
            self.cap_path.lineEdit().setPlaceholderText(
                "No captures found — use Browse or run AutoPwn first"
            )

    def _populate_wordlist_combo(
        self, combo: QComboBox, category: str = "password"
    ):
    ...
    def _make_log_tab(self) -> QWidget:
        ...
        self.log_table.setHorizontalHeaderLabels(
            ["Time", "Action", "Tool", "Status", "Details"]
        )
    ...
    def _ap_context_menu(self, pos):
        ...
        row = item.row()
        bssid = (
            self.ap_table.item(row, 0).text()
            if self.ap_table.item(row, 0)
            else ""
        )
        essid = (
            self.ap_table.item(row, 1).text()
            if self.ap_table.item(row, 1)
            else ""
        )
        channel = (
            self.ap_table.item(row, 2).text()
            if self.ap_table.item(row, 2)
            else ""
        )
        ...
        if bssid:
            ...
            copy_bssid.triggered.connect(
                lambda: QApplication.clipboard().setText(bssid)
            )
        if essid:
            copy_essid = menu.addAction(f"📋 Copy ESSID: {essid}")
            copy_essid.triggered.connect(
                lambda: QApplication.clipboard().setText(essid)
            )
        ...
        if bssid:
            ...
            set_target = menu.addAction(f"🎯 Set as Target")
            set_target.triggered.connect(
                lambda: self._set_target_from_menu(bssid)
            )
    ...
    def _show_system_status(self, status: dict):
        ...
        for tool, available in status.items():
            ...
            lbl = QLabel(
                f'<span style="color:{color}; font-size:13px;">{dot}</span>  {tool}'
            )
    ...
    def _run_manual_cmd(self):
        ...
        w.finished.connect(
            lambda r: self._term_print(r.stdout + r.stderr if r else "")
        )
        w.error.connect(lambda e: self._term_print(f"[ERROR] {e}"))
        self._start_worker(w)
    ...
    def _populate_recon(self, result: dict):
        ...
        for host in result.get("hosts", []):
            ...
            for port in host.get("ports", []):
                ...
                self.recon_table.setItem(
                    row, 1, QTableWidgetItem(str(port["port"]))
                )
                self.recon_table.setItem(
                    row, 2, QTableWidgetItem(port["state"])
                )
                self.recon_table.setItem(
                    row, 3, QTableWidgetItem(port["service"])
                )
                self.recon_table.setItem(
                    row, 4, QTableWidgetItem(port["version"])
                )
        self._term_print(
            f"[RECON] Found {self.recon_table.rowCount()} open ports."
        )
        if new_targets:
            self._update_all_target_comboboxes()
    ...
    def _do_deauth(self):
        ...
        w.finished.connect(
            lambda r: self._wifi_print(r.stdout if r else "done")
        )
        self._start_worker(w)

    def _do_crack_wpa(self):
        cap = self._get_wordlist_path(
            self.cap_path
        )  # works for any QComboBox with data
        wl = self._get_wordlist_path(self.wl_path)
        if not cap:
            QMessageBox.warning(
                self,
                "No Capture",
                "Select a .cap file first.\nUse 'Detect' to find captures from AutoPwn runs.",
            )
            return
        if not wl:
            QMessageBox.warning(
                self, "No Wordlist", "Select a wordlist first."
            )
            return
        self.crack_output.setPlainText(
            f"Cracking {cap.split('/')[-1]} with {wl.split('/')[-1]}…\n"
        )
    ...
    def _do_autopwn(self):
        ...
        if not iface_text:
            QMessageBox.warning(
                self, "No Interface", "Please select a Wi-Fi interface first."
            )
            return
        iface = iface_text.split()[0]
        ...
        if not wordlist:
            QMessageBox.warning(
                self,
                "No Wordlist",
                "No wordlist selected and none auto-detected.\nSelect one from the Wordlist dropdown.",
            )
            return

        self._term_print(
            f"[AUTOPWN] Triggered on interface {iface} with {wordlist.split('/')[-1]}"
        )
        self.autopwn_btn.setEnabled(False)
        w = WorkerThread(self.orch.auto_wifi_pwn, iface, wordlist)
        w.finished.connect(
            lambda r: self._term_print(
                f"[AUTOPWN] Workflow complete: {json.dumps(r)}"
            )
        )
        w.finished.connect(lambda _: self.autopwn_btn.setEnabled(True))
        w.error.connect(
            lambda e: (
                self._term_print(f"[ERROR] AutoPwn failed: {e}"),
                self.autopwn_btn.setEnabled(True),
            )
        )
        self._start_worker(w)
    ...
    def _do_crack_hash(self):
        ...
        self.crack_output.setPlainText(
            f"Cracking hash with {wl.split('/')[-1]}…\n"
        )
    ...
    def _browse(self, target_input: QLineEdit, pattern: str):
        path, _ = QFileDialog.getOpenFileName(
            self, "Select File", "/home/malcolm", pattern
        )
        if path:
    ...
    def _populate_ap_table(self, result: dict):
        ...
        for ap in aps:
            ...
            self.ap_table.setItem(
                row, 0, QTableWidgetItem(ap.get("bssid", ""))
            )
            self.ap_table.setItem(
                row, 1, QTableWidgetItem(ap.get("essid", ""))
            )
            self.ap_table.setItem(
                row, 2, QTableWidgetItem(str(ap.get("channel", "")))
            )
            self.ap_table.setItem(
                row, 3, QTableWidgetItem(ap.get("privacy", ""))
            )
            # Signal strength with visual bar
            pwr = ap.get("power", -100)
            bars = "█" * max(0, min(5, (pwr + 100) // 15)) + "░" * max(
                0, 5 - max(0, (pwr + 100) // 15)
            )
    ...
    def _ap_table_select(self, item):
        ...
        if bssid:
            ...
            try:
                ...
                pass
            self._wifi_print(
                f"[TARGET] Selected: {bssid} ({essid}) ch{channel}"
            )
    ...
    def _refresh_loot(self):
        ...
        for entry in loot["keys"]:
            lines.append(
                f"🔑 {entry['essid'] or entry['id']} → {entry['key']}  [{entry['method']}]"
            )
        self.loot_label.setText("\n".join(lines))
        self.loot_label.setStyleSheet(
            "color: #00ff88; font-size: 11px; font-family: 'JetBrains Mono';"
        )
    ...
    def _do_wifi_blitz(self):
        ...
        if not iface_text:
            QMessageBox.warning(
                self, "No Interface", "Select a Wi-Fi interface first."
            )
            return
        iface = iface_text.split()[0]
        wordlist = self._get_wordlist_path(self.wifi_wl_combo)
        ...
        if not wordlist:
            QMessageBox.warning(
                self,
                "No Wordlist",
                "Select a wordlist from the dropdown first.",
            )
            return
        self._term_print(
            f"[ONE-CLICK] 🔥 Wi-Fi Blitz on {iface} with {wordlist.split('/')[-1]}"
        )
        w = WorkerThread(self.orch.oneclick_wifi_blitz, iface, wordlist)
        w.finished.connect(
            lambda r: self._term_print(
                f"[ONE-CLICK] Wi-Fi Blitz finished: {len(r.get('cracked', []))} cracked"
            )
        )
        w.error.connect(
            lambda e: self._term_print(f"[ERROR] Wi-Fi Blitz failed: {e}")
        )
        self._start_worker(w)
    ...
    def _do_network_dominate(self):
        ...
        target, ok = QInputDialog.getItem(
            self,
            "Network Dominate",
            "Target range (e.g. 192.168.1.0/24):",
            targets,
            0,
            True,
        )
        ...
        w.finished.connect(
            lambda r: self._term_print(
                f"[ONE-CLICK] Network Dominate finished: {len(r.get('services',[]))} services found"
            )
        )
        w.error.connect(
            lambda e: self._term_print(f"[ERROR] Network Dominate failed: {e}")
        )
        self._start_worker(w)
    ...
    def _do_web_pwn(self):
        ...
        w.finished.connect(
            lambda r: self._term_print("[ONE-CLICK] Web Pwn finished")
        )
        w.error.connect(
            lambda e: self._term_print(f"[ERROR] Web Pwn failed: {e}")
        )
        self._start_worker(w)
    ...
    def _do_stealth_recon(self):
        ...
        targets = [""] + sorted(list(self.known_targets))
        target, ok = QInputDialog.getItem(
            self, "Stealth Recon", "Target domain/IP:", targets, 0, True
        )
        if not ok or not target:
            return
        ...
        w.finished.connect(
            lambda r: self._term_print("[ONE-CLICK] Stealth Recon finished")
        )
        w.error.connect(
            lambda e: self._term_print(f"[ERROR] Stealth Recon failed: {e}")
        )
        self._start_worker(w)
    ...
    def _on_kill_complete(self, summary: dict):
        ...
        self._term_print(
            f"🛑 Kill complete — {killed} processes killed, {restored} interfaces restored"
        )
        self._wifi_print(
            f"🛑 Kill complete — interfaces restored. Wi-Fi should reconnect."
        )
        self._restore_kill_btn()
    ...
    def _on_reboot_complete(self, _):
        ...
        self._term_print(
            f"[SYS] {len(self.known_targets)} saved targets preserved."
        )
    ...
    def _do_toggle_remote(self):
        ...
        if self.remote_server.is_running():
            ...
            self.remote_btn.setStyleSheet(
                self.remote_btn.styleSheet().replace(
    ...
    def _on_gui_remote_started(self, result):
        ...
        if result.get("success"):
            ...
            self._term_print(
                "║   Open the Browser URL on ANY device to see      ║"
            )
            self._term_print(
                "║   the full JAMES GUI — click, drag, everything.  ║"
            )
            self._term_print(
                "║                                                  ║"
            )
            self._term_print(
                "║   VNC clients: use the VNC address above.        ║"
            )
            self._term_print(
                "╚══════════════════════════════════════════════════╝"
            )
            ...
            self._term_print(
                "❌ GUI Remote failed to start. Check errors above."
            )
    ...
    def _export_log(self):
        path, _ = QFileDialog.getSaveFileName(
            self,
            "Export Log",
            "/home/malcolm/Desktop/james_log.json",
            "JSON (*.json)",
        )
        if path:
    ...
    def _on_task_update(self, entry):
        ...
        self.append_output.emit(
            f"[{entry.status.upper()}] {entry.action} ({entry.tool})"
        )
    ...
    def _term_print(self, text: str):
        ...
        elif "cracked" in text_lower and (
            "key" in text_lower or "password" in text_lower
        ):
    ...
    _TERM_HIGHLIGHT_RULES = [
        ...
        (
            ...
            r'<span style="color:#00f0ff;">\1</span>',
        ),
        # MAC addresses / BSSIDs
        (
            re.compile(r"([0-9A-Fa-f]{2}(?::[0-9A-Fa-f]{2}){5})"),
            r'<span style="color:#ff6b35;">\1</span>',
        ),
        # [ERROR] tags
        (
            re.compile(r"(\[ERROR\])"),
            r'<span style="color:#ff4757;font-weight:bold;">\1</span>',
        ),
        # [OK] / [+] tags
        (
            re.compile(r"(\[\+\]|\[OK\])"),
            r'<span style="color:#00ff88;font-weight:bold;">\1</span>',
        ),
        # [SYS] / [INFO] tags
        (
            re.compile(r"(\[SYS\]|\[INFO\])"),
            r'<span style="color:#5a9abf;">\1</span>',
        ),
        # [AUTOPWN] / [ONE-CLICK] tags
        (
            re.compile(r"(\[AUTOPWN\]|\[ONE-CLICK\]|\[TEST\])"),
            r'<span style="color:#ff6b35;font-weight:bold;">\1</span>',
        ),
        # Port numbers (e.g., 22/tcp, 80/tcp)
        (
            re.compile(r"(\d{1,5}/(?:tcp|udp))"),
            r'<span style="color:#ff6b35;">\1</span>',
        ),
    ]
        ...
        try:
            if self._targets_file.exists():
                ...
                if isinstance(data, list):
                    ...
                    if self.known_targets:
                        self._term_print(
                            f"[SYS] Loaded {len(self.known_targets)} saved targets"
                        )
        except Exception:
    ...
    def _save_targets(self):
        ...
        try:
            ...
            self._targets_file.write_text(
                json.dumps(sorted(self.known_targets))
            )
        except Exception:
            pass
